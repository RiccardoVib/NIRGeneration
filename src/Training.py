...
def train(**kwargs):
    ...
    # Load test dataset
    test_dataset = DataGeneratorPickles(data_dir, f"{dataset}_test.pickle", seq_len=seq_len)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

    train_dataset = DataGeneratorPickles(data_dir, f"{dataset}_train.pickle", seq_len=seq_len)
    ...
    x_test = test_dataset.x.reshape(len(test_loader), seq_len, 1)
    y_test = test_dataset.y.reshape(len(test_loader), seq_len, 1)
